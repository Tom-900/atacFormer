...
class BinVocab:
    def __init__(self, bin_file):
        ...
        self.vocab, self.token_to_ind_map, self.token_name_to_ind_map, \
            self.ind_to_token_map = self._create_vocab()
    ...
    def _create_vocab(self):
        ...
        return vocab, token_to_ind_map, token_name_to_ind_map, ind_to_token_map
...
token = bin_vocab.vocab["token"].tolist()
chr_id_ls = [t[0] for t in token]
pos_id_ls = [t[1] for t in token]
random_indices = random.sample(range(len(chr_id_ls)), 100 * 5000)
chr_id_vec = [chr_id_ls[i] for i in random_indices]
pos_id_vec = [pos_id_ls[i] for i in random_indices]

chr_id_vec = torch.tensor([chr_id_ls[i] for i in random_indices])
pos_id_vec = torch.tensor([pos_id_ls[i] for i in random_indices])

token = [(int(c), int(p)) for c, p in zip(chr_id_vec, pos_id_vec)]
ind = bin_vocab.token_to_ind(token)
