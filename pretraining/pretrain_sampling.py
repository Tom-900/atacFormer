...
sys.path.insert(0, "/users/s1155223034/my_project/atacFormer")
from data.databank import DataBank
from data.vocab import BinVocab
from model.model_sampling import TransformerModel
...
parser.add_argument(
    ...
    help="The sampling proportion for inaccessible ATAC bins in decoder prediction.",
)
parser.add_argument(
    "--loss-weight-cls",
    type=float,
...
parser.add_argument(
    "--use-class-emb",
    action="store_true",
    help="Whether to use class encoder. Default is False.",
)
parser.add_argument(
    "--nlayers-dna-enc",
    type=int,
    ...
    for batch, data_dict in enumerate(train_loader):
        ...
        with torch.cuda.amp.autocast(enabled=args.fp16):
            output_dict = model(
                    ...
                    use_cls=USE_CLS,
                    decoder_prop=sampling_prop,
                    )
            
            predictions = output_dict["predictions"] # list[(batch, sample_len_i, 3) for i in range(23)]
            formulated_targets = output_dict["formulated_targets"] # list[(batch, sample_len_i) for i in range(23)]
...
def evaluate(model: nn.Module, valid_loader: DataLoader, epoch: int) -> Dict[str, torch.Tensor]:
    """
...
def eval_and_save(
    ...
    val_loss, val_acc = evaluate(model, valid_loader, epoch=iter_or_epoch).values()
