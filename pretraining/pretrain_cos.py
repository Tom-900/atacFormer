...
#将训练的开始时间和当前 Git 提交的哈希值写入到 run.log 这个日志文件中
...
if args.load_model is not None:
    ...
    if args.local_rank in [0, -1]:
        #Save the new args to the save_dir where save_dir is Path object of the inputed arg.save_dir
        with open(save_dir / "args.json", "w") as f:
            json.dump(vars(args), f, indent=2)
...
def cosine_annealing(step, total_steps, max_value, min_value):
    """
    根据余弦退火策略计算当前值。
    
    参数:
        step: 当前步数（global iteration 或 batch）
        total_steps: 总步数（退火周期）
        max_value: 初始值（例如初始 loss_weight）
        min_value: 最小值（衰减到的目标值）
    
    返回:
        当前值（根据余弦函数计算）
    """
    return min_value + 0.5 * (max_value - min_value) * (1 + math.cos(math.pi * step / total_steps))
...
def train(model: nn.Module, 
    ...
    min_loss_weight = 150  # minimum loss weight
    total_steps = len(train_loader)  # number of batchs in one epoch
    current_loss_weight = initial_loss_weight
    state_weights = [1, (1 - args.masked_ratio) * current_loss_weight, args.masked_ratio * current_loss_weight]
    criterion = nn.CrossEntropyLoss(weight=torch.tensor(state_weights, dtype=torch.float).to(device))
    
    logger.info(f"Epoch {epoch}: Reset loss_weight to {current_loss_weight:.4f}")
    ...
    for batch, data_dict in enumerate(train_loader):
        ...
        current_loss_weight = cosine_annealing(batch, total_steps, initial_loss_weight, min_loss_weight)
        state_weights = [1, (1 - args.masked_ratio) * current_loss_weight, args.masked_ratio * current_loss_weight]
        criterion = nn.CrossEntropyLoss(weight=torch.tensor(state_weights, dtype=torch.float).to(device))
        if batch % 500 == 0 and batch >= 0:
            logger.info(f"Epoch {epoch}, Batch {batch}: Updated loss_weight to {current_loss_weight:.4f}")
        ...
        with torch.cuda.amp.autocast(enabled=args.fp16):
            ...
            mlm_loss = []
            for c in range(23):
                #shape:(batch_size * num_bins[i], 3) and (batch_size * num_bins[i]) respectively
        ...
        #Gradient Computation（avoid gradient vanishing)
        ...
        with torch.no_grad():
            ...
            for c in range(23):
                ...
                if token_c.sum() > 0:
                    ...
                    #number of correct predictions of the open bins which belongs to chromosome c 
                    # inside the cell sentence 
